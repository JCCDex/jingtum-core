...
#ifndef SKYWELL_CORE_CONFIG_H_INCLUDED
#define SKYWELL_CORE_CONFIG_H_INCLUDED
...
#include <type_traits>
#include <vector>
#include <boost/asio/ip/tcp.hpp> //  FIX: This include should not be here
#include <boost/filesystem.hpp> //  FIX: This include should not be here
...
public:
    ...
    // DEPRECATED
    boost::filesystem::path     VALIDATORS_FILE;        // As specifed in skywelld.cfg.

    /** List of Validators entries from skywelld.cfg */
...
public:
    ...
    std::vector<std::string>    IPS;                    // Peer IPs from skywelld.cfg.
    std::vector<std::string>    IPS_FIXED;              // Fixed Peer IPs from skywelld.cfg.
    std::vector<std::string>    SNTP_SERVERS;           // SNTP servers from skywelld.cfg.
    std::vector<std::string>    MYSQL_CONFIG;           // MYSQL config from skywelld.cfg. add by frank for mysql config
    ...
    */
    ...
    // Validation
    SkywellAddress               VALIDATION_SEED;
    SkywellAddress               VALIDATION_PUB;
    SkywellAddress               VALIDATION_PRIV;

    // Node/Cluster
    std::vector<std::string>    CLUSTER_NODES;
    SkywellAddress               NODE_SEED;
    SkywellAddress               NODE_PUB;
    SkywellAddress               NODE_PRIV;

    
    // Fee schedule (All below values are in fee units)
    std::uint32_t               FEE_DEFAULT;            // Default fee.
