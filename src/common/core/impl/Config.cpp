...
#include <common/core/ConfigSections.h>
#include <common/base/Log.h>
#include <common/json/json_reader.h>
#include <protocol/SystemParameters.h>
#include <services/net/HTTPClient.h>
...
#define DEFAULT_FEE_ACCOUNT_RESERVE     100*SYSTEM_CURRENCY_PARTS
#define DEFAULT_FEE_OWNER_RESERVE       10*SYSTEM_CURRENCY_PARTS
...
/** Parses a set of strings into boost::asio::ip::tcp::endpoint
      ...
      failed parse.
      @param out An OutputSequence to store the boost::asio::ip::tcp::endpoint list
...
void
parseAddresses (OutputSequence& out, 
                InputIterator first, 
                InputIterator last,
                beast::Journal::Stream stream = beast::Journal::Stream ())
{
    while (first != last)
    {
        ...
        std::vector<std::string> v;
        {
            ...
            boost::asio::ip::tcp::endpoint const addr (address, port);
        ...
        if (stream) stream << "Config: \"" << str << "\" is not a valid IP address.";
    }
}
...
void Config::loadFromString (std::string const& fileContents)
{
	...
	if (auto s = getIniFileSection(secConfig, SECTION_MYSQL_CONFIG))
		MYSQL_CONFIG = *s;
    ...
    if (getSingleSection (secConfig, SECTION_PEERS_MAX, strTemp))
        PEERS_MAX           = boost::lexical_cast <int> (strTemp);
    ...
    if (getSingleSection (secConfig, SECTION_VALIDATION_SEED, strTemp))
    {
        ...
        {
            VALIDATION_PUB = SkywellAddress::createNodePublic (VALIDATION_SEED);
            VALIDATION_PRIV = SkywellAddress::createNodePrivate (VALIDATION_SEED);
        }
    }
    ...
    if (getSingleSection (secConfig, SECTION_NODE_SEED, strTemp))
    {
        ...
        {
            NODE_PUB = SkywellAddress::createNodePublic (NODE_SEED);
            NODE_PRIV = SkywellAddress::createNodePrivate (NODE_SEED);
        }
    }

    if (getSingleSection (secConfig, SECTION_NETWORK_QUORUM, strTemp))
        NETWORK_QUORUM      = boost::lexical_cast<std::size_t> (strTemp);
    ...
    if (getSingleSection (secConfig, SECTION_FEE_ACCOUNT_RESERVE, strTemp))
        FEE_ACCOUNT_RESERVE = boost::lexical_cast<std::uint32_t> (strTemp);

    if (getSingleSection (secConfig, SECTION_FEE_OWNER_RESERVE, strTemp))
        FEE_OWNER_RESERVE   = boost::lexical_cast<std::uint32_t> (strTemp);
    ...
    if (getSingleSection (secConfig, SECTION_FEE_ACCOUNTID, strTemp))
    ...
    if (getSingleSection (secConfig, SECTION_LEDGER_HISTORY, strTemp))
    {
        ...
        if (strTemp == "full")
            ...
            LEDGER_HISTORY = boost::lexical_cast<std::uint32_t> (strTemp);
    }
    ...
    if (getSingleSection (secConfig, SECTION_FETCH_DEPTH, strTemp))
    {
        ...
        if (strTemp == "none")
            ...
            FETCH_DEPTH = boost::lexical_cast<std::uint32_t> (strTemp);
    ...
    if (getSingleSection (secConfig, SECTION_PATH_SEARCH_OLD, strTemp))
        PATH_SEARCH_OLD     = boost::lexical_cast<int> (strTemp);
    if (getSingleSection (secConfig, SECTION_PATH_SEARCH, strTemp))
        PATH_SEARCH         = boost::lexical_cast<int> (strTemp);
    if (getSingleSection (secConfig, SECTION_PATH_SEARCH_FAST, strTemp))
        PATH_SEARCH_FAST    = boost::lexical_cast<int> (strTemp);
    if (getSingleSection (secConfig, SECTION_PATH_SEARCH_MAX, strTemp))
        PATH_SEARCH_MAX     = boost::lexical_cast<int> (strTemp);
